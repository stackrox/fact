...
__always_inline static long process_fill_lineage(struct raw_event_t* event,
  ...
  for (; i < LINEAGE_MAX; i++) {
    ...
    raw_event_copy_u32(event, task->cred->uid.val);
...
__always_inline static int64_t process_fill(struct raw_event_t* event, bool use_bpf_d_path) {
  ...
  raw_event_copy_u32(event, task->loginuid.val);
  ...
  err = raw_event_copy_args(event, task);
  if (err != 0) {
  ...
  if (cg == NULL || raw_event_copy_str(event, cg) < 0) {
    bpf_printk("Failed to read cgroup");
    return -1;
  }

  raw_event_copy_u8(event, get_mount_ns() == host_mount_ns);

  err = process_fill_lineage(event, task, helper, use_bpf_d_path);
  if (err < 0) {
    return -1;
  }
